...
!Test
   ...
   if (rank.eq.0)then
      ...
      write(*,'(A24,F10.6)')  "Re                     :",Re
      write(*,'(A24,F10.4)')  "Ca                     :",Ca
...
!---------------------------------------------
   ...
   do j=local_start(2),local_end(2)
      do i=local_start(1),local_end(1)
         ...
         do iq=0,4
            gamma5 =  tv(iq)*(1.d0+3.d0*(ev(iq,1) * ua(i,j)+     &
                           ev(iq,2) * va(i,j)))
   ...
   enddo
      ...
      do i=local_start(1),local_end(1)
         ...
         forsx1=-3.d0*delta*sigma12*div1(i,j)*sqd1(i,j)*dx1(i,j)  
         forsy1=-3.d0*delta*sigma12*div1(i,j)*sqd1(i,j)*dy1(i,j)
...
!-----------------------------------------------------------
   ...
   do j=local_start(2)-ghost,local_end(2)+ghost
      do i=local_start(1)-ghost,local_end(1)+ghost
         ...
         dist1=2.d0*(sqrt((i-nx/2)**2+(j-ny/2)**2+0.d0)-r0)/delta


         phi1(i,j)=0.5-0.5*tanh(dist1)
...
!---------------------------------------------
   ...
   do j=local_start(2),local_end(2)
      do i=local_start(1),local_end(1)
         ...
         forsa11=2.d0*aa11(i,j)*dux(i,j)+aa21(i,j)*dvx(i,j)+aa12(i,j)*dvx(i,j)-1.d0/lambda*(aa11(i,j)-1.d0)
         forsa12=aa12(i,j)*dux(i,j)+aa11(i,j)*duy(i,j)+aa22(i,j)*dvx(i,j)+aa12(i,j)*dvy(i,j)-1.d0/lambda*(aa12(i,j))
         forsa21=aa21(i,j)*dux(i,j)+aa11(i,j)*duy(i,j)+aa22(i,j)*dvx(i,j)+aa21(i,j)*dvy(i,j)-1.d0/lambda*(aa21(i,j))
         forsa22=2.d0*aa22(i,j)*dvy(i,j)+aa21(i,j)*duy(i,j)+aa12(i,j)*duy(i,j)-1.d0/lambda*(aa22(i,j)-1.d0)
         do iq=0,4
            gamma5 =  tv(iq)*(1.d0+3.d0*(ev(iq,1) * ua(i,j)+     &
                           ev(iq,2) * va(i,j)))
            ...
            aaeq21= aa21(i,j) *  gamma5- 0.5d0 * forsa21*gamma5
            ...
            bb21(iq,i,j)=b21(iq,i,j)-1.d0/(0.5d0+tk)*(b21(iq,i,j)-aaeq21)+forsa21*gamma5
   ...
   do j=local_start(2),local_end(2)
      do i=local_start(1),local_end(1)
         ...
         forsax=(divx11(i,j)+divy21(i,j))*phi1(i,j)
         forsay=(divx12(i,j)+divy22(i,j))*phi1(i,j)
...
!---------------------------------------------
   ...
   if(ny.eq.local_end(2))then
      do i=local_start(1)-1,local_end(1)+1
         ...
         do iq=0,nq-1
            if(e(iq,2).lt.0)then
               fb (iq,i,ny+1) = fb (bc(iq),i+2*e(iq,1),ny-1)+2.d0*e(iq,1)*t(iq)*uu0/2*ny
   ...
   if(0.eq.local_start(2))then
      do i=local_start(1)-1,local_end(1)+1
         ...
         do iq=0,nq-1
            if(e(iq,2).gt.0)then
               fb (iq,i,-1) = fb (bc(iq),i+2*e(iq,1),1)-2.d0*e(iq,1)*t(iq)*uu0/2*ny
  ...
  enddo
   ...
   do j=local_start(2),local_end(2)
      do i=local_start(1),local_end(1)
         ...
         aaa11(i,j)=ratio*(aa11(i,j)-1.d0)
         aaa12(i,j)=ratio*aa12(i,j)
         aaa21(i,j)=ratio*aa21(i,j)
         aaa22(i,j)=ratio*(aa22(i,j)-1.d0)
      enddo
   enddo
